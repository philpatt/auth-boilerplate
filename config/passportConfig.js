...
passport.use(new facebookStrategy({
	...
	//see if we have a valid email address we can use to identify the user
	...
	db.user.findOne({
		...
		if(existingUser && facebookEmail){
			...
			//the person is new and that is ok, just create an entry for them in users table
			//parse user's name
			...
			db.user.findOrCreate({
				...
				if(wasCreated){
					...
					//they were not new after all - we just need to update the token (for a new log in session)
